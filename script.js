...
// WhatsApp links are real anchors, so ordering works even if JavaScript is disabled.
// Update the selected size before navigating.
document.querySelectorAll(".order-whatsapp").forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    btn.href = `https://api.whatsapp.com/send?phone=919496739361&text=${encodeURIComponent(message)}`;
  });
});
